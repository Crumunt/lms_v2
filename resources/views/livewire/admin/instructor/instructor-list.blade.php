<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <div class="text-3xl font-bold text-green-600">
                        {{ $instructors->where('status', 'approved')->count() }}
                    </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <div class="text-3xl font-bold text-purple-600">{{ $instructors->sum('taught_courses_count') }}
                    </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div
                                        ...
                                        <span
                                            class="text-white text-sm font-medium">{{ substr(strtoupper($instructor->detail?->full_name), 0, 2) }}</span>
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $instructor->detail?->full_name }}
                                        </div>
                                        <div class="text-sm text-gray-500">{{ $instructor->detail?->address }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    ...
                                    {{ ucfirst($instructor->detail?->status->name) }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    ...
                                    <button wire:click="deleteInstructor('{{ $instructor->id }}')"
                                        wire:confirm="Are you sure you want to delete this instructor? This action cannot be undone."
